...
import (
	...
	"github.com/sonroyaalmerol/go-smb-server/internal/md4"
)
...
// NTHash returns the 16-byte NT password hash used as the base key for NTLMv2.
// Keeping this hash lets an embedded server authenticate SMB clients without
...
// NTOWFv2FromNTHash derives the domain/user-specific NTLMv2 response key.
func NTOWFv2FromNTHash(ntHash []byte, user, domain string) []byte {
...
func NTOWFv2(password, user, domain string) []byte {
	return NTOWFv2FromNTHash(NTHash(password), user, domain)
}
