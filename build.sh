...
OUT="${1:-build}"
mkdir -p "$OUT"
CGO_ENABLED=0 GOOS=linux GOARCH=arm GOARM=5 go build -trimpath -ldflags='-s -w' -o "$OUT/WiFiFiles.server" server.go mobile.go webdav.go webdav_ui.go qr.go ftp.go smb.go filenames.go library.go auth.go logging.go paths.go upload.go
CLANG="${CLANG:-clang}"
