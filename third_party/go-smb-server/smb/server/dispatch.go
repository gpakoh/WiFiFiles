...
func (c *conn) handleNegotiate(msg []byte, hdr *wire.Header) uint32 {
	...
	resp := wire.NegotiateResponse{
		SecurityMode:    wire.SigningEnabled | wire.SigningRequired,
...
func (c *conn) handleSessionSetup(ctx context.Context, msg []byte, hdr *wire.Header) uint32 {
	...
	if result.Identity != nil {
		...
		if c.dialect >= wire.DialectSMB30 {
			sess.signingKey = signing.DeriveSigningKey(result.SessionKey)
			sess.signingAlgo = signing.AlgoAESCMAC
		} else {
			sess.signingKey = append([]byte(nil), result.SessionKey...)
			sess.signingAlgo = signing.AlgoHMACSHA256
		}
