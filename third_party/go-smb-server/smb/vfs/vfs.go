...
func (h *localHandle) SetInfo(_ context.Context, req *SetInfoRequest) error {
	...
	if req.Attributes != nil && *req.Attributes&0x02 != 0 {
		// FAT-backed PocketBook storage does not support chmod. Treat the
		// DOS read-only attribute as best effort instead of failing the SMB op.
		_ = os.Chmod(h.path, 0400)
	}
	return nil
}
